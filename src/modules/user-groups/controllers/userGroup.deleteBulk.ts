...
export const deleteUserGroupsBulkController = asyncHandler(async (req, res) => {
	...
	const data = await deleteUserGroupsBulk(parsed.data!.ids);

	const code =
		data.processedIds.length === 0
			? 'USER_GROUPS_BULK_SOFT_DELETE_NONE'
			: data.notFoundIds.length > 0
				? 'USER_GROUPS_BULK_SOFT_DELETE_PARTIAL'
				: 'USER_GROUPS_BULK_SOFT_DELETE_SUCCESS';
