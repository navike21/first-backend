...
describe('portfolioUpdateController', () => {
	it('calls updatePortfolio and returns 200', async () => {
		vi.mocked(updatePortfolio).mockResolvedValue({ id: '1', slug: 'project' } as never);
		const req = { params: { id: '1' }, body: { status: 'published' } } as unknown as Request;
		const res = makeRes();
	...
	it('calls next with error on invalid body', async () => {
		const req = { params: { id: '1' }, body: { serviceIds: 'not-an-array' } } as unknown as Request;
		const res = makeRes();
		const next = vi.fn();
