...
export async function listPageRevisions(pageId: string) {
	...
	const revisions = await PageRevisionModel.find({ pageId })
		.sort({ createdAt: -1 })
		.lean();
...
async function checkSlugConflict(
	...
	const entries = Object.entries((slug as Record<string, string>) ?? {}).filter(
		([, v]) => v?.trim(),
	);
...
export async function restorePageRevision(
	...
	const revision = await PageRevisionModel.findOne({
		id: revisionId,
		pageId,
	}).lean();
	...
	const restoredParentId =
		(snapshot.parentId as string | null | undefined) ?? null;
	...
	for (const field of SNAPSHOT_FIELDS) {
		if (field in snapshot)
			(doc as unknown as Record<string, unknown>)[field] = snapshot[field];
	}
