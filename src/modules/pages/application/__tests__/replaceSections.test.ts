...
vi.mock('@Modules/pages/infrastructure/PageModel', () => ({ default: { findOne: vi.fn() } }));
vi.mock('@Modules/pages/infrastructure/PageRevisionModel', () => ({ default: { create: vi.fn() } }));
...
const revisionCreateMock = PageRevisionModel.create as unknown as ReturnType<typeof vi.fn>;
...
describe('replaceSections', () => {
	...
	it('throws PageNotFoundError when the page does not exist', async () => {
		...
		await expect(replaceSections('missing', { sections: [] }, 'u1')).rejects.toThrow(PageNotFoundError);
	});
	...
	it('assigns sectionId to new sections and normalizes order by index', async () => {
		...
		await replaceSections(
			...
			{
				sections: [
					{ type: 'columns', order: 99, settings: { columns: 2 }, content: { columns: [] } },
					{
	...
	it('records one page revision per save', async () => {
		...
		expect(revisionCreateMock.mock.calls[0][0]).toMatchObject({ pageId: 'page-1', createdBy: 'u1' });
	});
});
