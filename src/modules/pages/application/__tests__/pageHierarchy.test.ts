...
import { PageInvalidParentError, PageParentNotFoundError } from '@Modules/pages/domain/errors/PageErrors';
...
describe('assertValidParent', () => {
	it('does nothing when no parent is being set', async () => {
		await expect(assertValidParent('page-1', undefined)).resolves.toBeUndefined();
		await expect(assertValidParent('page-1', null)).resolves.toBeUndefined();
	});

	it('rejects a page becoming its own parent', async () => {
		await expect(assertValidParent('page-1', 'page-1')).rejects.toThrow(PageInvalidParentError);
	});
	...
	it('rejects when the candidate parent is actually a descendant (cycle)', async () => {
		...
		vi.mocked(PageModel.findOne).mockReturnValueOnce(
			mockLeanFindOne({ id: 'page-3', parentId: 'page-1' }) as never,
		);

		await expect(assertValidParent('page-1', 'page-3')).rejects.toThrow(PageInvalidParentError);
	});

	it('accepts a valid, non-cyclic parent chain', async () => {
		vi.mocked(PageModel.findOne)
			.mockReturnValueOnce(mockLeanFindOne({ id: 'page-3', parentId: 'page-4' }) as never)
			.mockReturnValueOnce(mockLeanFindOne({ id: 'page-4', parentId: null }) as never);

		await expect(assertValidParent('page-1', 'page-3')).resolves.toBeUndefined();
	});

	it('throws PageParentNotFoundError when the candidate parent does not exist', async () => {
		vi.mocked(PageModel.findOne).mockReturnValueOnce(mockLeanFindOne(null) as never);

		await expect(assertValidParent('page-1', 'ghost')).rejects.toThrow(PageParentNotFoundError);
	});
});

describe('computeFullPath', () => {
	it('joins the parent path and own slug for a given language', () => {
		const result = computeFullPath({ en: 'team', es: 'equipo' }, { en: 'about', es: 'nosotros' });
...
describe('cascadeRecomputeFullPath', () => {
	it('recomputes fullPath for descendants using the parent chain', async () => {
		vi.mocked(PageModel.find)
			.mockReturnValueOnce(
				mockLeanFind([{ id: 'child-1', slug: { en: 'team' }, parentId: 'root' }]) as never,
			)
			.mockReturnValueOnce(mockLeanFind([{ id: 'root', fullPath: { en: 'about' } }]) as never)
			.mockReturnValueOnce(mockLeanFind([]) as never);
	...
	it('stops when there are no more descendants', async () => {
		vi.mocked(PageModel.find).mockReturnValueOnce(mockLeanFind([]) as never);
