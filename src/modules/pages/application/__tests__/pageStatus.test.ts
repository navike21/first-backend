...
import { resolveEffectiveStatus, withEffectiveStatus, publicVisibilityFilter } from '@Modules/pages/application/pageStatus';
...
describe('resolveEffectiveStatus', () => {
	...
	it('treats a scheduled page whose date has passed as published', () => {
		...
		expect(resolveEffectiveStatus({ status: 'scheduled', scheduledAt: past })).toBe('published');
	});

	it('keeps a scheduled page whose date is in the future as scheduled', () => {
		const future = new Date(Date.now() + 60_000);
		expect(resolveEffectiveStatus({ status: 'scheduled', scheduledAt: future })).toBe('scheduled');
	});
...
describe('withEffectiveStatus', () => {
	it('attaches the computed effectiveStatus alongside the original fields', () => {
		...
		const result = withEffectiveStatus({ id: '1', status: 'scheduled', scheduledAt: past });
