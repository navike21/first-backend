...
describe('subscriberListAll', () => {
  it('calls listAllSubscribers with pagination params', async () => {
    vi.mocked(listAllSubscribers).mockResolvedValue({ data: [], meta: {} } as never);
    const req = { query: { limit: '5', page: '2', status: 'active' } } as unknown as Request;
    const res = makeRes();
    const next = vi.fn();
    await subscriberListAll(req, res, next);
    expect(listAllSubscribers).toHaveBeenCalledWith({ limit: 5, page: 2, status: 'active' });
  ...
  it('uses defaults when query params are absent', async () => {
    ...
    const next = vi.fn();
    await subscriberListAll(req, res, next);
    expect(listAllSubscribers).toHaveBeenCalledWith({ limit: 10, page: 1, status: undefined });
  });
});
