...
describe('SubscriberModel', () => {
	...
	it('email validator returns true for a valid email', () => {
		const emailPath = (SubscriberModel as any).schema.path('contactInformation.email');
		const validate = emailPath.validators[1].validator as (v: string) => boolean;
	...
	it('dateOfBirth validator returns true for a valid date', () => {
		const dobPath = (SubscriberModel as any).schema.path('personalInformation.dateOfBirth');
