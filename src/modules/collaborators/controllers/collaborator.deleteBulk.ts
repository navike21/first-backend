...
export const deleteCollaboratorsBulkController = asyncHandler(async (req, res) => {
	...
	const data = await deleteCollaboratorsBulk(parsed.data!.ids);

	const code =
		data.processedIds.length === 0
			? 'COLLABORATORS_BULK_SOFT_DELETE_NONE'
			: data.notFoundIds.length > 0
				? 'COLLABORATORS_BULK_SOFT_DELETE_PARTIAL'
				: 'COLLABORATORS_BULK_SOFT_DELETE_SUCCESS';
