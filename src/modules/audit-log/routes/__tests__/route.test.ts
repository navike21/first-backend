...
vi.mock('@Constants/permissions', () => ({
	PERMISSIONS: { AUDIT_LOGS_READ: 'audit-logs:read', AUDIT_LOGS_MANAGE: 'audit-logs:manage' },
}));
...
vi.mock('../../controllers/auditLog.list', () => ({
	auditLogListController: (_req: unknown, res: { json: (v: unknown) => void }) =>
		res.json({ ok: true }),
}));

vi.mock('../../controllers/auditLog.getById', () => ({
	auditLogGetByIdController: (_req: unknown, res: { json: (v: unknown) => void }) =>
		res.json({ ok: true }),
}));
