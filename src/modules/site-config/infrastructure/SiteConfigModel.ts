...
const siteConfigSchema = new Schema<SiteConfigDocument>(
	{
		...
		header: {
			variant: { type: String, enum: HEADER_VARIANTS, default: SITE_CONFIG_DEFAULTS.header.variant },
			...
			mobile: {
				...
				menuIconPosition: { type: String, enum: ['left', 'right'], default: 'right' },
			},
		},
		footer: {
			variant: { type: String, enum: FOOTER_VARIANTS, default: SITE_CONFIG_DEFAULTS.footer.variant },
			columns: { type: Number, enum: [3, 4], default: 4 },
			showSocial: { type: Boolean, default: true },
			showNewsletter: { type: Boolean, default: false },
			copyright: { type: Schema.Types.Mixed, default: () => SITE_CONFIG_DEFAULTS.footer.copyright },
		},
		layout: {
